...
<?php
$items = array();
$items['title'] = __d(__INVESTMENT_LOCALE,'edit_company');
$items['link'] = array('title'=>__d(__INVESTMENT_LOCALE,'company_list'),'url'  =>__SITE_URL.'admin/'.__INVESTMENT.'/links/index');
...
<div class="col-md-6">
	...
	$user_image = '';
		...
		if(fileExistsInPath(__INVESTMENT_IMAGE_PATH.$image ) && $image != '' ){
			$user_image = $this->Html->image('/'.__INVESTMENT_IMAGE_URL.$image,array('width' =>$width,'height'=>$height,'id'=>'blog_thumb_image_'.$this->request->data['Company']['id']));
		}
		else
		{
			$user_image = $this->Html->image('/'.__INVESTMENT.'/new_blog.png',array('width' =>$width,'height'=>$height,'alt'   =>''));
		}
		echo $user_image;
