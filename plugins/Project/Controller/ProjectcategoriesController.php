...
class ProjectcategoriesController extends ProjectAppController
{
	...
	function admin_add()
	{
		...
		if ($this->request->is('post')) {
			...
			if ($this->Projectcategory->save($this->request->data)) {
				$project_category_id = $this->Projectcategory->getLastInsertID();
				$this->Projectcategory->Projectcategorytranslation->recursive = -1;

				$this->request->data['Projectcategorytranslation']['project_category_id'] = $project_category_id;
				$this->request->data['Projectcategorytranslation']['language_id'] = $this->Cookie->read(__ADMIN_LANG_DEFAULT_ID);
				$this->request->data['Projectcategorytranslation']['title'] = trim($this->request->data['Projectcategorytranslation']['title']);
				$this->Projectcategory->Projectcategorytranslation->create();
				if ($this->Projectcategory->Projectcategorytranslation->save($this->request->data))
	...
	function admin_edit($id = null)
	{
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if ($this->Projectcategory->save($this->request->data)) {
				...
				$options = array();
				$options['conditions'] = array(
					"Projectcategorytranslation.project_category_id" => $id,
					"Projectcategorytranslation.language_id" => $this->Cookie->read(__ADMIN_LANG_DEFAULT_ID)
				);
				$count = $this->Projectcategory->Projectcategorytranslation->find('count', $options);
				/*
				* @blog translate
				*/
				if ($count == 0) {
					$this->Projectcategory->Projectcategorytranslation->recursive = -1;
					$this->request->data['Projectcategorytranslation']['project_category_id'] = $id;
					$this->request->data['Projectcategorytranslation']['language_id'] = $this->Cookie->read(__ADMIN_LANG_DEFAULT_ID);
					$this->request->data['Projectcategorytranslation']['title'] = trim($this->request->data['Projectcategorytranslation']['title']);
					$this->Projectcategory->create();
					if (!$this->Projectcategory->Projectcategorytranslation->save($this->request->data))
					...
					$ret = $this->Projectcategory->Projectcategorytranslation->updateAll(
						array('Projectcategorytranslation.title' => '"' . trim($this->request->data['Projectcategorytranslation']['title']) . '"'
						),
						array('Projectcategorytranslation.project_category_id' => $id, 'language_id' => $this->Cookie->read(__ADMIN_LANG_DEFAULT_ID))
					);
					if (!$ret) {
						$this->Redirect->flashWarning(__d(__PROJECT_LOCALE, 'the_project_category_could_not_be_saved'), array('action' => 'index'));
					}
	...
	public function _indexgetPath($category_id, $title)
	{
		...
		$query = $this->Projectcategory->find('all', $options);
